 ...
 */
const mongoGetGoodsReceivings = async (queryParams) => {
  ...
  return paginate(GoodsReceiving, {
    ...
    populate: [
      ...
      { path: 'items.productId', select: 'name sku golongan' },
 ...
 */
const mongoGetGoodsReceivingById = async (id) => {
  const gr = await GoodsReceiving.findById(id)
    ...
    .populate('items.product', 'name sku golongan nie manufacturer')
 ...
 */
const mongoCreateGoodsReceiving = async (data, userId) => {
  ...
  if (data.purchaseOrderId) {
    ...
    if (po.status !== PO_STATUS.SENT) {
      throw ApiError.badRequest('PO harus berstatus sent');
    }
 ...
 */
const mongoUpdateGoodsReceiving = async (id, data, userId) => {
  const gr = await GoodsReceiving.findById(id);
  if (!gr) {
    throw ApiError.notFound('Goods receiving not found');
  }
 ...
 */
const mongoDeleteGoodsReceiving = async (id) => {
  const gr = await GoodsReceiving.findById(id);
  if (!gr) {
    throw ApiError.notFound('Goods receiving not found');
  }
 ...
 */
const mongoVerifyGoodsReceiving = async (id, notes, userId) => {
  const gr = await GoodsReceiving.findById(id);
  if (!gr) {
    throw ApiError.notFound('Goods receiving not found');
  }
  ...
  try {
    ...
    logger.error(`Failed to create journal/invoice for GR ${gr._id}: ${err.message}`);
  }
...
/**
 * Get available POs for receiving (status: sent)
 */
const mongoGetAvailablePOs = async (queryParams) => {
...
const getGoodsReceivings = (q) => config.dbProvider === 'mysql' ? mysqlGetGoodsReceivings(q) : mongoGetGoodsReceivings(q);
const getStats = () => config.dbProvider === 'mysql' ? mysqlGetStats() : mongoGetStats();
const getGoodsReceivingById = (id) => config.dbProvider === 'mysql' ? mysqlGetGoodsReceivingById(id) : mongoGetGoodsReceivingById(id);
const createGoodsReceiving = (data, userId) => config.dbProvider === 'mysql' ? mysqlCreateGoodsReceiving(data, userId) : mongoCreateGoodsReceiving(data, userId);
const updateGoodsReceiving = (id, data, userId) => config.dbProvider === 'mysql' ? mysqlUpdateGoodsReceiving(id, data, userId) : mongoUpdateGoodsReceiving(id, data, userId);
const deleteGoodsReceiving = (id) => config.dbProvider === 'mysql' ? mysqlDeleteGoodsReceiving(id) : mongoDeleteGoodsReceiving(id);
const verifyGoodsReceiving = (id, notes, userId) => config.dbProvider === 'mysql' ? mysqlVerifyGoodsReceiving(id, notes, userId) : mongoVerifyGoodsReceiving(id, notes, userId);
const getAvailablePOs = (q) => config.dbProvider === 'mysql' ? mysqlGetAvailablePOs(q) : mongoGetAvailablePOs(q);
